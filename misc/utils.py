...
def plt_fig_to_tensor(inputs):
    ...
    inputs_np = np.fromstring(canvas.tostring_rgb(),
...
def gifs_to_tensor(gifs):
    ...
    # cat w
    # --> T*b, cols, c, h, w
    stacked_gifs = stacked_gifs.view(-1, cols, c, h, w)
    stacked_gifs = stacked_gifs.permute(
        0, 1, 4, 2, 3).contiguous()  # --> T*b, cols, w, c, h
    # --> T*b, cols*w, c, h --> T*b, c, h, cols*w
    w_cat = stacked_gifs.view(-1, cols*w, c, h).permute(0, 2, 3, 1)
    # cat h
    w_cat = w_cat.view(T, b, c, h, cols*w)
    w_cat = w_cat.permute(0, 1, 3, 2, 4).contiguous()  # --> T, b, h, c, cols*w
    h_cat = w_cat.view(-1, b*h, c, cols*w).permute(0, 2, 1,
                                                   3).contiguous()  # --> T, c, b*h, cols*w
